	...
	case MSGOBJ2SYS_KOUTAI2:
		{
            ...
            for (int i = 0; i < (MAXNUM_TEAM - 1); ++i) {
				if (charobjid[team][next_act[i]] == 0){
					continue;
				}
				pdat = (CGObject*)GetGObject(charobjid[team][next_act[i]]);
				if (!pdat){
				...
				if (pdat->Message(GOBJMSG_KOUTAI, charobjid[team][cidx]))
				{
					...
					active_character[team] = next_act[i];//"アクティブ" キャラクター更新
					g_system.PopSysTag();
					return(TRUE);					//成功
				}
